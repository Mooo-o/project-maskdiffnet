...
class GeometricStructureEmbedding(nn.Module):
    def __init__(self, hidden_dim, sigma_d, sigma_a, angle_k, reduction_a='max'):
        ...
        self.alpha = nn.Parameter(torch.ones(1)) #新增
    ...
    def get_embedding_indices(self, points):
        ...
        dist_map = torch.sqrt(pairwise_distance(points, points) + 1e-10)  # (B, N, N)
    ...
    def forward(self, points):
        ...
        # 这样单纯的直接相加会有一些问题
        embeddings = self.alpha * d_embeddings + self.beta * a_embeddings  #新增 加了学习参数
        #print(f"alpha shape: {self.alpha.item()} beta shape: {self.beta.item()}")
...
class GeometricTransformer(nn.Module):
    def __init__(
        ...
        self.transformer = RPEConditionalTransformer(
            embed_dim, depth, blocks, hidden_dim, num_heads, dropout=dropout, activation_fn=activation_fn, return_attention_scores=True
        )
    ...
    def forward(
        ...
        """print(f"[DEBUG] ref_points shape: {ref_points.shape}")  ([1, N, 3])
        print(f"[DEBUG] src_points shape: {src_points.shape}")     ([1, M, 3])
        print(f"[DEBUG] ref_feats shape before projection: {ref_feats.shape}")   ([1, N, 1024])
        print(f"[DEBUG] src_feats shape before projection: {src_feats.shape}")   ([1, M, 1024])"""

        ref_embeddings = self.embedding(ref_points)  # 得到特征嵌入
        src_embeddings = self.embedding(src_points)
        """print(f"[DEBUG] ref_embeddings shape: {ref_embeddings.shape}")   ([1, N, N, 1024])
        print(f"[DEBUG] src_embeddings shape: {src_embeddings.shape}")      ([1, M, M, 1024])"""

        ref_feats = self.in_proj(ref_feats) # 连接层，拓展到隐藏维度
        src_feats = self.in_proj(src_feats)  
        """ print(f"[DEBUG] ref_feats shape after projection: {ref_feats.shape}")  ([1, N, 256])
        print(f"[DEBUG] src_feats shape after projection: {src_feats.shape}")      ([1, M, 256])"""

        ref_feats, src_feats, attention_score = self.transformer(
            ref_feats,
            src_feats,
            ref_embeddings,
            src_embeddings,
        ...
        """ print(f"[DEBUG] ref_feats shape after transformer: {ref_feats.shape}")  ([1, N, 256])
        print(f"[DEBUG] src_feats shape after transformer: {src_feats.shape}")      ([1, M, 256])"""

        ref_feats = self.out_proj(ref_feats)
        src_feats = self.out_proj(src_feats)
        """ print(f"[DEBUG] ref_feats shape after output projection: {ref_feats.shape}")  ([1, N, 256])
        print(f"[DEBUG] src_feats shape after output projection: {src_feats.shape}")      ([1, M, 256])"""

        return ref_feats, src_feats, attention_score
