...
def eval_one_epoch(args, cfg, logger):
    ...
    for scene_root in scene_roots:
        ...
        for file_name in file_names:
            ...
            '''# ======================================================
            # [筛选逻辑] 仅当内点率 > 50% (0.5) 且重叠率 > 30% (0.3) 时导出
            # 你可以根据需要调整这个阈值，比如 inlier_ratio > 0.8
            # ======================================================
            if 0.5 < inlier_ratio < 0.9 and overlap < 0.7:  # and overlap < 0.6  inlier_ratio > 0.8
                ...
                # 文件名增加指标信息，方便本地挑选
                # 例如: scene_000_010_IR0.85_OV0.45
                base_name = f"{scene_abbr}_{ref_frame:03d}_{src_frame:03d}_IR{inlier_ratio:.2f}_OV{overlap:.2f}"

                # 目标点云（ref）
                pcd_ref = o3d.geometry.PointCloud()
                ref_points = data_dict['ref_corr_points']
                ...
                src_points = data_dict['src_corr_points']
                ...
                # 计算 inlier / outlier 掩码并保存
                dist_thresh = getattr(cfg.eval, 'acceptance_radius', 0.05)
                ...
                np.savez(
                    ...
                    overlap=overlap,           # 这里直接用前面算出来的 float
                    inlier_ratio=inlier_ratio, # 把 IR 也存进去
                    dist_thresh=dist_thresh,
                )
                
                print(f"Exported good match: {base_name}")
            # ======================================================
            '''
