...
from geotransformer.utils.open3d import registration_with_ransac_from_correspondences, draw_geometries, make_open3d_point_cloud, get_color
...
def eval_one_epoch(args, cfg, logger):
    ...
    for scene_root in scene_roots:
        ...
        for file_name in file_names:
            ...
            # Save point clouds before and after registration
            # if args.verbose:
            ...
            if gt_indices[ref_frame, src_frame] != -1:
                ...
                if error > 0.2:
                    # 使用 source 和 target 原始点云（配准之前的）
                    ref_points = data_dict['ref_points']   # 原始参考点云
                    src_points = data_dict['src_points']   # 原始源点云

                    # 应用 estimated_transform 到 src_points
                    src_points_aligned = (estimated_transform[:3, :3] @ src_points.T + estimated_transform[:3, 3:4]).T
                    src_points_gt_aligned = (transform[:3, :3] @ src_points.T + transform[:3, 3:4]).T
                    ...
                    src_aligned_pcd = make_open3d_point_cloud(src_points_aligned)
                    src_aligned_pcd.estimate_normals()
                    src_aligned_pcd.paint_uniform_color(get_color("custom_blue"))
                    
                    src_gt_aligned_pcd = make_open3d_point_cloud(src_points_gt_aligned)
                    src_gt_aligned_pcd.estimate_normals()
                    src_gt_aligned_pcd.paint_uniform_color(get_color("custom_blue"))

                    # 创建保存目录
                    save_dir = osp.join(cfg.registration_dir, benchmark, scene_name, 'ply')
                    os.makedirs(save_dir, exist_ok=True)
                    
                    # 保存 ply 文件
                    o3d.io.write_point_cloud(osp.join(save_dir, f'{ref_frame}_{src_frame}_ref.ply'), ref_pcd)
                    o3d.io.write_point_cloud(osp.join(save_dir, f'{ref_frame}_{src_frame}_src.ply'), src_pcd)
                    o3d.io.write_point_cloud(osp.join(save_dir, f'{ref_frame}_{src_frame}_src_aligned.ply'), src_aligned_pcd)
                    o3d.io.write_point_cloud(osp.join(save_dir, f'{ref_frame}_{src_frame}_src_gt_aligned.ply'), src_gt_aligned_pcd) 
