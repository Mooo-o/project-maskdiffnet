...
class Trainer(EpochBasedTrainer):
    def __init__(self, cfg):
        ...
        model = create_model(cfg).cuda()  # 模型创建
        ...
        scheduler = CosineLRScheduler(  #新增
            optimizer,
            t_initial=cfg.optim.max_epoch,  # 总共训练 epoch 数
            lr_min=1e-5,                    # 最小学习率
            warmup_t=5,                     # warmup 前 5 个 epoch
            warmup_lr_init=2e-5,           # warmup 初始学习率
            warmup_prefix=True
        )
